...
public class RamseyGraph implements GraphProperty {
    ...
    public boolean run(Graph graph) {
        ...
        // Фиксированные и «серые» рёбра
        List<Edge> fixedRed   = new ArrayList<>();
        List<Edge> fixedBlue  = new ArrayList<>();
        List<Edge> grayEdges  = new ArrayList<>();

        for (Edge e : graph.getEdgeList()) {
            if (e.getColor() == Color.RED)      fixedRed.add(e);
            else if (e.getColor() == Color.BLUE) fixedBlue.add(e);
            else           grayEdges.add(e);
        }

        int m = grayEdges.size();
        int total = 1 << m;  // 2^m раскрасок

        // Перебор всех раскрасок "серых" рёбер
        for (int mask = 0; mask < total; mask++) {
            Map<Integer, Set<Integer>> redAdj  = initAdj(verts);
            Map<Integer, Set<Integer>> blueAdj = initAdj(verts);

            // Добавляем фиксированные рёбра
            paintEdges(fixedRed,  redAdj);
            paintEdges(fixedBlue, blueAdj);


            for (int i = 0; i < m; i++) {
                Edge e = grayEdges.get(i);
                if (((mask >> i) & 1) == 1) paintEdge(e, redAdj);
                else                        paintEdge(e, blueAdj);
            }


            boolean hasR = hasClique(redAdj, getIds(verts), r);
            boolean hasB = hasClique(blueAdj, getIds(verts), s);
            if (!hasR && !hasB) {
    ...
    // Инициализация пустого adj-списка
    private Map<Integer, Set<Integer>> initAdj(List<Vertex> verts) {
        Map<Integer, Set<Integer>> adj = new HashMap<>();
        for (Vertex v : verts) adj.put(v.getId(), new HashSet<>());
        return adj;
    }

    // Добавление одного ребра (двунаправленного) в adj
    private void paintEdge(Edge e, Map<Integer, Set<Integer>> adj) {
        int u = e.getSource(), v = e.getTarget();
        adj.get(u).add(v);
        adj.get(v).add(u);
    }


    private void paintEdges(List<Edge> edges, Map<Integer, Set<Integer>> adj) {
        for (Edge e : edges) paintEdge(e, adj);
    }

    private List<Integer> getIds(List<Vertex> verts) {
        List<Integer> ids = new ArrayList<>(verts.size());
        for (Vertex v : verts) ids.add(v.getId());
        return ids;
    }
